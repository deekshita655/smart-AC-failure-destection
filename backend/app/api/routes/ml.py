...
def _service() -> MLFailureMiningService:
    # Lazy construction means the SentenceTransformer model is not downloaded
    # or loaded merely because the API starts. Existing application behavior is
    # unchanged until an ML endpoint is explicitly used.
...
def train_failure_mining(db: Session = Depends(get_db)):
    """Train the supplied SentenceTransformer + KMeans pipeline on ticket notes."""
...
def ml_status():
    service = _service()
    return success({
        "trained": service.is_fitted,
        "embedding_model": service.model_name,
        "n_clusters": service.n_clusters,
        "artifact_dir": service.artifact_dir.as_posix(),
    })
...
def predict_ticket_cluster(ticket_id: str, db: Session = Depends(get_db)):
    """Assign one service ticket to the nearest learned failure-theme cluster."""
...
def store_ticket_cluster_result(ticket_id: str, db: Session = Depends(get_db)):
    """Persist the ML cluster assignment in the existing AI result table.

    This does not overwrite technician diagnosis or ground truth. Cluster IDs
    are stored alongside those fields through the existing AIAnalysisResult
    contract so downstream analytics can distinguish ML output from labels.
    """
    ticket = db.query(ServiceTicket).filter(ServiceTicket.ticket_id == ticket_id).first()
    if not ticket:
        raise_error("TICKET_NOT_FOUND", "Service ticket not found.")
