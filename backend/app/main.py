...
app = FastAPI(
    title="Smart AC Failure Intelligence & Predictive Maintenance API",
    version="0.1.0",
    description="Backend for the Smart AC Failure Intelligence and Predictive "
                "Maintenance platform. See /architecture.md in the project root "
                "for the full architecture and API catalog.",
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # CONFIGURATION_PENDING: restrict to frontend origin in production
...
async def api_error_handler(request: Request, exc: APIError):
    ...
    return JSONResponse(status_code=exc.status_code, content=error_body(exc, rid))
...
for router in [
    auth.router, users.router, devices.router, service_tickets.router, images.router,
