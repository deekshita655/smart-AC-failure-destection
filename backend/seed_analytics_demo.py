...
try:
    ...
    devices_spec = [
        ("DEMO-AN-001", "AC-Model-X1", "SR-001"),
        ("DEMO-AN-002", "AC-Model-X1", "SR-001"),
        ("DEMO-AN-003", "AC-Model-X2", "SR-002"),
        ("DEMO-AN-004", "AC-Model-X2", "SR-002"),
        ("DEMO-AN-005", "AC-Model-Z2", "SR-007"),
        ("DEMO-AN-006", "AC-Model-Z2", "SR-008"),
    ]
    ...
    anchor = (latest_existing.date if latest_existing else datetime.now()).replace(
    ...
    for index, (days_ago, device_id, symptom, failure_mode, component, department, severity, fix) in enumerate(ticket_specs, 1):
        ...
        if db.query(ServiceTicket).filter(ServiceTicket.ticket_id == ticket_id).first():
            continue
        ...
        ticket = ServiceTicket(
            ...
            fix_text=fix,
            technician_diagnosis_failure_mode=failure_mode,
            technician_diagnosis_component=component,
            technician_diagnosis_department=department,
            severity=severity,
            ground_truth_failure_mode=failure_mode,
            ground_truth_component=component,
            ground_truth_department=department,
        ...
        confidence = round(0.78 + (index % 5) * 0.04, 2)
        db.add(AIAnalysisResult(
            ticket_id=ticket.id,
            model_name="smart-ac-demo-classifier",
            model_version="demo-v1",
            prediction_timestamp=ticket_date + timedelta(minutes=5),
            predicted_failure_mode=predicted_mode,
            predicted_component=predicted_component,
            predicted_department=predicted_department,
            confidence=min(confidence, 0.96),
            suggested_action=fix,
            raw_result_json={"demo": True},
            normalized_result_json={
                "failure_mode": predicted_mode,
    ...
    health_specs = [
        ("DEMO-AN-001", 58, 0.72, "AT_RISK"),
        ("DEMO-AN-002", 76, 0.44, "HEALTHY"),
        ("DEMO-AN-003", 69, 0.55, "AT_RISK"),
        ("DEMO-AN-004", 42, 0.88, "CRITICAL"),
        ("DEMO-AN-005", 91, 0.18, "HEALTHY"),
        ("DEMO-AN-006", 63, 0.61, "AT_RISK"),
    ]
    ...
    # Four historical health points per device make the predictive trend chart useful.
    ...
    for device_id, base in base_scores.items():
        ...
        for offset in range(4, 0, -1):
            ...
            if db.query(DeviceHealth).filter(DeviceHealth.device_id == device.id, DeviceHealth.timestamp == ts).first():
                continue
    ...
    predictive_specs = [
        ("DEMO-AN-001", "Compressor degradation", 0.91, "HIGH", "CONFIRMED"),
        ("DEMO-AN-002", "Refrigerant pressure drift", 0.84, "MEDIUM", "PENDING"),
        ("DEMO-AN-003", "Fan vibration", 0.73, "MEDIUM", "FALSE_POSITIVE"),
        ("DEMO-AN-004", "PCB instability", 0.94, "CRITICAL", "CONFIRMED"),
        ("DEMO-AN-005", "Sensor drift", 0.82, "LOW", "PENDING"),
        ("DEMO-AN-006", "Compressor overload", 0.89, "HIGH", "CONFIRMED"),
    ]
    for device_id, issue, confidence, risk, outcome in predictive_specs:
        ...
        if outcome in ("CONFIRMED", "PENDING") and not db.query(PreventiveTicket).filter(PreventiveTicket.predictive_event_id == event.id).first():
            db.add(PreventiveTicket(
                ...
                status="PENDING" if outcome == "PENDING" else "OPEN",
        ...
        if not db.query(Anomaly).filter(Anomaly.device_id == device.id, Anomaly.detected_at == anchor).first():
            db.add(Anomaly(
                device_id=device.id,
                detected_at=anchor,
                anomaly_score=round(1 - next(h for d, h, _, _ in health_specs if d == device_id) / 100, 2),
                description=issue,
                resolved=outcome == "FALSE_POSITIVE",
            ))
