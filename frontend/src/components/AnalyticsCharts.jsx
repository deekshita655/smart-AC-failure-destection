...
export function BarChart({ data = [], labelKey, valueKey = 'count', title, limit = 8 }) {
  ...
  return <div className="chart-card">{title && <h3>{title}</h3>}<div className="bar-chart">{rows.map((r, i) => <div className="bar-row" key={`${r[labelKey]}-${i}`}><span className="bar-label" title={r[labelKey]}>{r[labelKey] ?? 'UNKNOWN'}</span><div className="bar-track"><div className="bar-fill" style={{ width: `${(Number(r[valueKey]) / max) * 100}%` }} /></div><span className="bar-value">{r[valueKey]}</span></div>)}{!rows.length && <div className="empty-chart">No data available</div>}</div></div>
}
...
export function LineChart({ data = [], xKey, yKey = 'count', title }) {
  ...
  return <div className="chart-card">{title && <h3>{title}</h3>}{rows.length ? <div className="line-wrap"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={title || 'Trend chart'}>{[0, .5, 1].map((ratio, i) => { const y = top + plotHeight - ratio * plotHeight; return <g key={i}><line x1={left} y1={y} x2={width-right} y2={y} className="chart-gridline" /><text x={left-8} y={y+4} textAnchor="end" className="chart-y-label">{Math.round(max * ratio)}</text></g> })}<polyline points={polyline} className="line-path" fill="none" />{points.map((p, i) => <g key={i}><circle cx={p.x} cy={p.y} r="5" className="line-dot" /><text x={p.x} y={p.y-10} textAnchor="middle" className="chart-point-label">{p.value}</text><text x={p.x} y={height-14} textAnchor="middle" className="chart-x-label">{p.label}</text></g>)}</svg></div> : <div className="empty-chart">No trend data available</div>}</div>
}
