...
export default function TechnicianDashboard() {
  ...
  async function lookupDevice() {
    ...
    try {
      ...
      const a = await api(`/analytics/devices/${data.device.device_id}`)
      setAnalytics(a)
  ...
  async function createTicket() {
    ...
    try {
      ...
      const data = await api('/service-tickets', {
        method: 'POST',
        body: {
          ticket_id: newTicketId, device_id: device.device_id,
          date: new Date().toISOString(), symptom_text: symptomText,
          fix_text: fixText, severity,
        },
      })
      setTicket(data)
  ...
  async function analyzeTicket() {
    ...
    try {
      ...
      setAiResult(data)
      const cmp = await api(`/service-tickets/${ticket.ticket_id}/comparison`)
      setComparison(cmp)
    } catch (e) { setError(e.message) }
  }
  ...
  async function sendChat() {
    ...
    try {
      const data = await api('/chat/message', {
        method: 'POST',
        body: { message: userMsg, device_id: device?.device_id, ticket_id: ticket?.ticket_id },
      })
      setChatLog(prev => [...prev, { role: 'bot', text: data.reply }])
    } catch (e) {
      setChatLog(prev => [...prev, { role: 'bot', text: `[${e.code || 'ERROR'}] ${e.message}` }])
    }
  }
  ...
  return (
    <Layout>
      ...
      <p className="subtitle">Identify a device, log a service report, and run AI failure analysis.</p>
      ...
      {analytics && (
        <div className="card">
          ...
          <div className="grid">
            ...
            {analytics.health && (
              <div><div className="stat">{analytics.health.health_score}</div><div className="stat-label">Health score</div></div>
            )}
          </div>
          <h2>Recent tickets</h2>
          <table>
            <thead><tr><th>Ticket</th><th>Date</th><th>Symptom</th><th>Status</th></tr></thead>
            <tbody>
              {analytics.recent_tickets.map(t => (
                <tr key={t.ticket_id}>
                  <td>{t.ticket_id}</td><td>{new Date(t.date).toLocaleDateString()}</td>
                  <td>{t.symptom_text}</td><td>{t.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {device && (
        <div className="card">
          ...
          <label>Ticket ID (optional, auto-generated if blank)</label>
          <input value={ticketId} onChange={e => setTicketId(e.target.value)} />
          <label>Symptom</label>
          <textarea rows={2} value={symptomText} onChange={e => setSymptomText(e.target.value)} />
          <label>Fix applied (optional)</label>
          <textarea rows={2} value={fixText} onChange={e => setFixText(e.target.value)} />
      ...
      {ticket && (
        <div className="card">
          <h2 style={{ marginTop: 0 }}>3. AI Analysis</h2>
          <button className="amber" onClick={analyzeTicket}>Run AI analysis</button>
          {aiResult && (
            <div style={{ marginTop: 14 }}>
              ...
              {comparison && (
                <p style={{ fontSize: 13 }}>
                  <strong>Technician vs AI:</strong>{' '}
                  {comparison.overall_match === null ? 'No technician diagnosis entered for comparison.' :
                    comparison.overall_match ? <span className="badge ok">MATCH</span> : <span className="badge danger">MISMATCH</span>}
                </p>
              )}
            </div>
          )}
      ...
      {device && (
        <div className="card">
          <h2 style={{ marginTop: 0 }}>Assistant (Gemini)</h2>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={chatMsg} onChange={e => setChatMsg(e.target.value)}
                   ...
                   placeholder="Ask about this device or ticket..." style={{ marginBottom: 0 }} />
